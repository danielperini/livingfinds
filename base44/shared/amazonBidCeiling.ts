...
export function clampAmazonBid(value: unknown, ceiling = AMAZON_BID_CEILING_BRL): number | unknown {
  ...
  const safeCeiling = Math.min(AMAZON_BID_CEILING_BRL, Math.max(0.02, Number(ceiling) || AMAZON_BID_CEILING_BRL));
  return Math.round(Math.min(bid, safeCeiling) * 100) / 100;
}

export function enforceBidCeilingOnPayload(path: string, method: string, payload: any): any {
  if (!['POST', 'PUT'].includes(String(method).toUpperCase()) || payload == null) return payload;
  const entityKey = path.includes('adGroups') ? 'adGroups' : path.includes('keywords') ? 'keywords' : null;
  if (!entityKey) return payload;

  const clamp = (item: any) => {
    if (!item || typeof item !== 'object') return item;
    const result = { ...item };
    if ('bid' in result) result.bid = clampAmazonBid(result.bid);
    if ('defaultBid' in result) result.defaultBid = clampAmazonBid(result.defaultBid);
    return result;
  };
