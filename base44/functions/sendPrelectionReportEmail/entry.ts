 ...
 * Envia email comparando métricas reais vs metas configuradas.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!prelection && body.data?.id) {
      // Chamado via automação de entidade — body.data é o registro atualizado
      prelection = body.data;
    }
    if (!prelection) {
      // Buscar o mais recente completado
    ...
    // Tacos (pode vir de goal_status se calculado)
    const tacosStatus = gs.tacos || 'no_data';

    // Motor efetividade: % de metas no verde
    const allStatuses = [gs.acos, gs.roas, gs.cpc, gs.budget].filter(s => s && s !== 'no_data');
    const okCount = allStatuses.filter(s => s === 'ok').length;
    const motorEffectiveness = allStatuses.length > 0 ? Math.round(okCount / allStatuses.length * 100) : 0;

    // Efetividade do motor: texto descritivo
...
<body style="font-family:Inter,Arial,sans-serif;background:#f1f5f9;margin:0;padding:24px;">
  <div style="max-width:620px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding:28px 32px;">
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px 20px;margin-bottom:24px;text-align:center;">
        <p style="margin:0 0 4px;font-size:24px;font-weight:800;color:#0f172a;">${motorEffectiveness}%</p>
        <p style="margin:0;font-size:13px;color:#475569;">Metas atingidas pelo motor diário</p>
        <p style="margin:8px 0 0;font-size:13px;">${effectivenessText}</p>
      </div>
    ...
    await base44.asServiceRole.integrations.Core.SendEmail({
      to: user.email,
      subject: `📊 Preleção Semanal ${weekLabel} — Motor ${motorEffectiveness}% efetivo | ACoS ${pct(realAcos)} vs meta ${pct(targetAcos)}`,
      body: html,
    });

    return Response.json({ ok: true, sent_to: user.email, week: weekLabel, motor_effectiveness: motorEffectiveness });
