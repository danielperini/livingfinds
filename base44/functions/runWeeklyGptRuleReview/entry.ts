 ...
 * Mantém compatibilidade com a função legada, que já usa OpenAI/GPT,
 * e normaliza a autoria/auditoria das regras criadas.
 *
 * Não existe aprovação manual: backtest + shadow + governança determinística
 * continuam sendo o gate de promoção. Toda alteração aceita é apenas sinalizada
 * no feed como "Alterado pela IA".
 */
  ...
  try {
    ...
    if (aid && reviewId) {
      ...
      for (const rule of rules) {
        ...
        const previous = await base44.asServiceRole.entities.OptimizationDecision.filter({
          amazon_account_id: aid,
          idempotency_key: `${aid}|gpt_weekly_rule_change|${reviewId}|${rule.rule_key}`,
        }, '-created_at', 1).catch(() => []);
