...
const requestedAsins = (body:any) => {
  const values = [body?.asin, ...(Array.isArray(body?.required_asins) ? body.required_asins : [])];
  return [...new Set(values.map(normAsin).filter((asin) => /^B[A-Z0-9]{9}$/.test(asin)))];
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requiredAsins = requestedAsins(body);
    ...
    for (const item of items) {
      ...
      if (existing) {
        ...
        byAsin.set(asin, { ...existing, ...patch });
        updated++;
      } else {
    ...
    const requiredResults = requiredAsins.map((asin) => {
      const product:any = byAsin.get(asin) || null;
      return {
        asin,
        found_in_fba_inventory: inventoryAsins.has(asin),
        product_persisted: !!product,
        product_id: product?.id || null,
        quantity: product ? num(product.fba_inventory ?? product.available_quantity ?? product.total_quantity) : null,
      };
    });
    const missingRequiredAsins = requiredResults.filter((item) => !item.found_in_fba_inventory).map((item) => item.asin);

    const completedAt = new Date().toISOString();
    await base44.asServiceRole.entities.SyncExecutionLog.create({
      amazon_account_id: body.amazon_account_id,
      operation: 'sync_product_catalog_v2', status: missingRequiredAsins.length ? 'warning' : 'success', trigger_type: body.trigger_type || 'manual',
      started_at: startedAt, completed_at: completedAt, records_processed: created + updated,
      result_summary: JSON.stringify({ pages, inventory_asins: items.length, created, updated, corrected, costs_loaded: costsLoaded, pending_cost_confirmation: pendingCostConfirmation, required_asins: requiredResults, missing_required_asins: missingRequiredAsins }).slice(0, 4000),
      error_message: missingRequiredAsins.length ? `ASIN(s) não retornado(s) pelo inventário FBA da SP-API: ${missingRequiredAsins.join(', ')}` : null,
    }).catch(() => {});
    ...
    return Response.json({
      ok: true,
      pages,
      inventory_asins: items.length,
      created,
      updated,
      corrected_from_out_of_stock: corrected,
      costs_loaded: costsLoaded,
      pending_cost_confirmation: pendingCostConfirmation,
      required_asins: requiredResults,
      missing_required_asins: missingRequiredAsins,
    });
  } catch (error:any) {
