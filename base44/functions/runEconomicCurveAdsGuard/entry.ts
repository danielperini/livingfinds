...
function abc(rows: Array<{ asin: string; value: number }>) {
  const sorted = rows.filter(row => row.asin && row.value > 0).sort((a, b) => b.value - a.value);
  ...
  for (const row of sorted) {
    ...
    out.set(row.asin, before < 0.80 ? 'A' : before < 0.95 ? 'B' : 'C');
    cumulative += row.value;
  }
  return out;
}

function matrixState(salesCurve: string, profitCurve: string) {
  ...
  return map[`${salesCurve}${profitCurve}`] || 'TEST';
}

function riskFactor(state: string) {
...
function betaPosterior(priorClicks: number, priorOrders: number, clicks: number, orders: number, fallbackCvr: number) {
  const pc = Math.max(0, priorClicks);
  const po = clamp(priorOrders, 0, pc);
  const historicalCvr = pc > 0 ? po / pc : clamp(fallbackCvr, 0.005, 0.50);
  const strength = clamp(pc, 8, 30);
  const alpha = 1 + historicalCvr * strength + Math.max(0, orders);
  const beta = 1 + (1 - historicalCvr) * strength + Math.max(0, clicks - orders);
...
function normalCdfApprox(z: number) {
  return clamp(1 / (1 + Math.exp(-1.702 * z)), 0, 1);
}

function reductionPct(lossRatio: number, state: string, merOver: boolean) {
  let pct = statePriority(state) >= 8 ? 0.35 : statePriority(state) >= 6 ? 0.30 : statePriority(state) >= 4 ? 0.25 : 0.20;
  if (lossRatio >= 2.5) pct = 0.60;
  else if (lossRatio >= 1.75) pct = Math.max(pct, 0.45);
  else if (lossRatio >= 1.25) pct = Math.max(pct, 0.35);
  if (merOver) pct = Math.min(0.60, pct + 0.05);
  return pct;
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      const merTarget = resolveTargetMer(settings, body);
      const productByAsin = new Map<string, any>(products.filter((p: any) => s(p.asin)).map((p: any) => [s(p.asin).toUpperCase(), p] as [string, any]));
      const econByAsin = new Map<string, any>(economics.filter((e: any) => s(e.asin)).map((e: any) => [s(e.asin).toUpperCase(), e] as [string, any]));
      const campaignById = new Map<string, any>(campaigns.map((c: any) => [s(c.campaign_id || c.amazon_campaign_id || c.id), c] as [string, any]));
      ...
      const revenueRows30: Array<{ asin: string; value: number }> = [];
      const profitRows: Array<{ asin: string; value: number }> = [];
      for (const product of products) {
        const asin = s(product.asin).toUpperCase();
        if (!asin) continue;
        const econ = econByAsin.get(asin);
        revenueRows30.push({ asin, value: Math.max(0, revenue30.get(asin) || n(product.total_sales_30d)) });
        const windowProfit = n(econ?.profit_after_ads_14d, NaN);
        const unitProfit = n(econ?.profit_after_ads ?? product.profit_after_ads, 0);
        const profit = Number.isFinite(windowProfit) && windowProfit !== 0 ? windowProfit : unitProfit * Math.max(0, n(product.total_units_30d));
        profitRows.push({ asin, value: Math.max(0, profit) });
      }
      const salesCurve = abc(revenueRows30);
      ...
      for (const term of terms) {
        ...
        if (rowDate !== today || ageMin(updated) > FRESHNESS_MINUTES) continue;

        const campaignId = s(term.campaign_id || term.amazon_campaign_id);
        const campaign = campaignById.get(campaignId);
        if (!campaign || !enabled(campaign)) continue;
        const asin = s(term.advertised_asin || term.asin || campaign.asin).toUpperCase();
        ...
        const sameSkuVerified = term.same_sku_attribution_verified === true;
        const adsSales = sameSkuVerified ? n(term.same_sku_sales) : n(term.sales_1d);
        const orders = sameSkuVerified ? n(term.same_sku_orders) : n(term.orders_1d);
        const clicks = n(term.clicks);
        if (spend <= 0 || clicks <= 0) continue;
        const cpc = spend / clicks;

        const keywordText = s(term.keyword_text || term.keyword || term.search_term).toLocaleLowerCase('pt-BR');
        const keywordId = s(term.keyword_id || term.amazon_keyword_id);
        const keyword = activeKeywords.find((k: any) =>
          (keywordId && s(k.keyword_id || k.id) === keywordId) ||
          (s(k.campaign_id) === campaignId && s(k.ad_group_id) === s(term.ad_group_id) && s(k.keyword_text || k.keyword).toLocaleLowerCase('pt-BR') === keywordText)
        );
        if (!keyword) continue;
        ...
        const profitClass = profitCurve.get(asin) || 'UNAVAILABLE';
        const curveState = profitClass === 'UNAVAILABLE' ? 'TEST' : matrixState(salesClass, profitClass);
        ...
        const maxProfitableSpend = Math.max(0, n(econ?.maximum_profitable_ad_spend ?? product.maximum_ad_spend_per_order, margin));
        ...
        const statisticalClickCeiling = cvrTarget > 0 && cvrTarget < 1 ? Math.ceil(Math.log(0.05) / Math.log(1 - cvrTarget)) : Infinity;
        const economicClickCeiling = cpc > 0 ? Math.max(2, Math.floor(lossBudget / cpc)) : Infinity;
        const dryClickCeiling = Math.max(2, Math.min(statisticalClickCeiling, economicClickCeiling));

        const priorClicks = Math.max(0, n(keyword.clicks));
        const priorOrders = Math.max(0, n(keyword.orders ?? keyword.purchases));
        const productCvrRaw = n(product.conversion_rate_30d, 5);
        const fallbackCvr = productCvrRaw > 1 ? productCvrRaw / 100 : productCvrRaw || 0.05;
        const post = betaPosterior(priorClicks, priorOrders, clicks, orders, fallbackCvr);
        const probabilityBelowSustainable = post.sd > 0 && cvrTarget > 0 ? normalCdfApprox((cvrTarget - post.mean) / post.sd) : 0;
        ...
        const protectedWinner = keyword.protected_high_performance === true ||
          (sameSkuVerified && orders > 0 && adsSales > 0 && currentAcos !== null && currentAcos <= safeAcos);
        ...
        const merCurveTrigger = merOver && statePriority(curveState) >= 7 && loss > 0;
        if (!economicTrigger && !sequentialTrigger && !bayesianTrigger && !merCurveTrigger && !noAdCapacity) continue;

        const pct = reductionPct(lossRatio, curveState, merOver);
        const proposedBid = bid(Math.max(currentBid * 0.40, Math.min(currentBid * (1 - pct), safeCpc > 0 ? safeCpc : currentBid)));
        if (proposedBid >= currentBid) continue;
        const entityId = s(keyword.keyword_id || keyword.id);
        const idempotencyKey = `economic-curve-guard:${accountId}:${entityId}:${today}:${proposedBid.toFixed(2)}`;
        if (!entityId || activeKeys.has(idempotencyKey)) continue;

        const reasonCode = noAdCapacity ? 'ASIN_NO_AD_CAPACITY'
        ...
        candidates.push({
          score: statePriority(curveState) * 100 + Math.min(99, lossRatio * 10) + (merOver ? 20 : 0),
          campaign, keyword, term, campaignId, asin, entityId, currentBid, proposedBid, pct, spend, adsSales,
          orders, clicks, cpc, sameSkuVerified, salesClass, profitClass, curveState, safeAcos, breakEvenAcos,
          margin, maxProfitableSpend, lossBudget, loss, projectedNextClickLoss, post, probabilityBelowSustainable,
          safeCpc, currentAcos, reasonCode, idempotencyKey,
        });
      }
      ...
      for (const item of candidates.slice(0, Math.min(MAX_ACTIONS, Math.max(1, n(body.max_actions, MAX_ACTIONS))))) {
        const now = new Date();
        const executeBefore = new Date(now.getTime() + FRESHNESS_MINUTES * 60000).toISOString();
        const rationale = `Guardrail econômico ABC: ASIN ${item.asin}, curva vendas ${item.salesClass}/lucro ${item.profitClass} (${item.curveState}); loss budget R$ ${item.lossBudget.toFixed(2)}, perda R$ ${item.loss.toFixed(2)}, próximo clique R$ ${item.projectedNextClickLoss.toFixed(2)}; ACoS seguro ${item.safeAcos.toFixed(2)}%; CVR posterior ${(item.post.mean * 100).toFixed(2)}%, limite inferior ${(item.post.low95 * 100).toFixed(2)}%, CPC seguro R$ ${item.safeCpc.toFixed(2)}; MER/TACoS ${mer === null ? 'indisponível' : `${(mer * 100).toFixed(2)}%`} vs meta ${(merTarget * 100).toFixed(2)}%.`;
        await base44.asServiceRole.entities.OptimizationDecision.create({
          ...
          asin: item.asin,
          sku: s(item.keyword.sku || item.term.advertised_sku || item.term.sku || productByAsin.get(item.asin)?.sku),
          action: 'reduce_bid',
          decision_type: 'economic_curve_ads_guard',
          rationale,
          reason_code: item.reasonCode,
          current_value: item.currentBid,
          proposed_value: item.proposedBid,
          value_before: item.currentBid,
          value_after: item.proposedBid,
          change_pct: -Math.round(item.pct * 100),
          confidence: clamp(0.70 + Math.min(0.25, item.probabilityBelowSustainable * 0.25), 0, 0.99),
          risk: item.curveState === 'CORE_WINNER' ? 'medium' : statePriority(item.curveState) >= 7 ? 'critical' : 'high',
          ...
          ...(item.currentAcos == null ? {} : { current_acos: r2(item.currentAcos) }),
          target_acos: r2(item.safeAcos),
          ...(item.post.mean > 0 ? { expected_clicks_per_order: r2(1 / item.post.mean) } : {}),
          no_conversion_click_multiple: item.orders === 0 && item.post.mean > 0 ? r2(item.clicks / (1 / item.post.mean)) : 0,
          maximum_acquisition_spend: r2(item.maxProfitableSpend || item.margin),
    ...
    return Response.json({
      ...
      policy: {
        ...
        profit_curve: 'ABC por lucro pós-Ads real disponível',
        same_sku_protection: true,
        ...
        execution: 'OptimizationDecision EXPEDITED_QUEUE P1 com validade máxima de 45 minutos',
