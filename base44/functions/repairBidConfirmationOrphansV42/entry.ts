...
const CONFIRMABLE = new Set(['executed', 'confirming']);
...
async function resolveKeyword(base44: any, row: any): Promise<any | null> {
  ...
  if (row.keyword_id) {
    const direct = await base44.asServiceRole.entities.Keyword.filter({
...
Deno.serve(async (req) => {
  try {
    ...
    const statuses = ['executed', 'confirming'];
    ...
    const byRemoteEntity = new Map<string, any[]>();
    for (const row of rows) {
      ...
      const group = byRemoteEntity.get(key) || [];
      group.push(row);
      byRemoteEntity.set(key, group);
    }
    ...
    for (const group of byRemoteEntity.values()) {
      ...
      if (!CONFIRMABLE.has(String(latest.status || ''))) continue;
      if (!attempted(latest)) {
        skippedNoAttempt++;
        continue;
      }
      ...
      if (keyword) {
        const remoteKeywordId = String(keyword.keyword_id || keyword.keywordId || '');
        if (remoteKeywordId) {
          patch.keyword_id = remoteKeywordId;
          if (String(latest.entity_type || '') === 'keyword') patch.entity_id = remoteKeywordId;
          resolvedIds++;
        }
      ...
      normalized++;
      if (latest.amazon_account_id) touchedAccounts.add(String(latest.amazon_account_id));
    }

    // Não reenvia nenhum bid. Apenas solicita a leitura remota canônica.
    const confirmations: any[] = [];
    for (const aid of touchedAccounts) {
      const response = await base44.asServiceRole.functions.invoke('confirmExecutedDecisions', {
        _service_role: true,
        amazon_account_id: aid,
        trigger_type: 'repair_bid_confirmation_orphans_v42',
    ...
    return Response.json({
      ...
      superseded,
      skipped_no_amazon_attempt: skippedNoAttempt,
      confirmations,
