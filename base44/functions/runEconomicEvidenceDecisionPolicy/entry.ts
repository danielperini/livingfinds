...
Deno.serve(async(request)=>{
 try{
  ...
  for(const account of accounts as any[]){
   ...
   const settings:any=(settingsRows as any[])[0]||{},defaultTarget=Math.max(1,n(settings.target_acos??settings.acos_target,15));
   const productByAsin=new Map<string,any>((products as any[]).filter((p:any)=>s(p.asin)).map((p:any)=>[s(p.asin).toUpperCase(),p] as [string,any]));
   const econByAsin=new Map<string,any>((economics as any[]).filter((e:any)=>s(e.asin)).map((e:any)=>[s(e.asin).toUpperCase(),e] as [string,any]));
   const campaignById=new Map<string,any>((campaigns as any[]).map((c:any)=>[s(c.campaign_id||c.amazon_campaign_id||c.id),c] as [string,any]));
   ...
   if(body.dry_run!==true){
    for(const row of selected){
     ...
     const existing=await base44.asServiceRole.entities.OptimizationDecision.filter({amazon_account_id:aid,idempotency_key:idem},'-created_at',1).catch(()=>[]); if((existing as any[]).length)continue;
