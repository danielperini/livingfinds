...
Deno.serve(async(request)=>{
 try{
  ...
  const accounts:any[]=body.amazon_account_id?await base44.asServiceRole.entities.AmazonAccount.filter({id:body.amazon_account_id},null,1):await base44.asServiceRole.entities.AmazonAccount.filter({status:'connected'},null,100);
  const results:any[]=[];
  for(const account of accounts){
   ...
   const settingsRows:any[]=loaded[0]||[],products:any[]=loaded[1]||[],economics:any[]=loaded[2]||[],terms:any[]=loaded[3]||[],campaigns:any[]=loaded[4]||[];
   const settings:any=settingsRows[0]||{},defaultTarget=Math.max(1,n(settings.target_acos??settings.acos_target,15));
   ...
   for(const term of terms){
    ...
    const campaignId=s(term.campaign_id||term.amazon_campaign_id),campaign:any=campaignById.get(campaignId); if(!campaign||!active(campaign))continue;
   ...
   const selected:any[]=candidates.sort((a:any,b:any)=>b.evidenceScore-a.evidenceScore).slice(0,Number(body.max_candidates||100)); let persisted=0,refreshed=0;
   if(body.dry_run!==true){for(const row of selected){
    const idem=`${aid}|economic_evidence_shadow|${today}|${row.campaign_id}|${row.search_term}|${row.zone}`,existing:any[]=await base44.asServiceRole.entities.OptimizationDecision.filter({amazon_account_id:aid,idempotency_key:idem},'-created_at',1).catch(()=>[]),now=new Date().toISOString();
    const patch:any={amazon_account_id:aid,decision_type:'economic_evidence_shadow',entity_type:'search_term',entity_id:row.search_term||row.campaign_id,asin:row.asin,campaign_id:row.campaign_id,action:`shadow_${low(row.zone)}`,rationale:`SHADOW econômico/contextual · ${row.rationale}; contextual_arm=${row.contextual_arm}; expected_reward=${n(row.contextual_expected_reward).toFixed(2)}; uncertainty=${n(row.contextual_uncertainty).toFixed(2)}. Não executa Amazon.`,risk:row.allowPause?'medium':'low',requires_approval:false,approval_status:'shadow_only',status:'shadow',queue_status:'not_queued',idempotency_key:idem,source_function:'runEconomicEvidenceDecisionPolicy',execution_result:'Hypothetical action only; canonical deterministic engine remains execution owner.',economic_zone:row.zone,evidence_level:row.evidenceLevel,evidence_score:row.evidenceScore,reward:row.reward,break_even_acos_pct:row.break_even_acos_pct,target_acos_pct:row.target_acos_pct,recommended_bid_adjustment_pct:row.bidAdjustmentPct,contextual_arm:row.contextual_arm,updated_at:now};
    if(existing.length){await base44.asServiceRole.entities.OptimizationDecision.update(existing[0].id,patch).catch(()=>null);refreshed++;}else{await base44.asServiceRole.entities.OptimizationDecision.create({...patch,created_at:now}).catch(()=>null);persisted++;}
   }}
