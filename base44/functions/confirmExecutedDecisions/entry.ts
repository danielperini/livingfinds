...
function confirmationFailureState(decision: any) {
  ...
  if (ambiguous && confirmationAttempts < maxConfirmationAttempts) {
    return {
      ...
      confirmation_status: 'propagating',
      confirmation_attempt_count: confirmationAttempts + 1,
      last_attempt_at: new Date().toISOString(),
    };
  }
  return {
    status: ambiguous ? 'failed_final' : 'failed',
    queue_status: 'failed',
    confirmation_attempt_count: confirmationAttempts + 1,
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < kwIds.length; i += 50) {
      ...
      for (const kw of (data?.keywords || [])) amazonKwById.set(String(kw.keywordId), kw);
    ...
    for (let i = 0; i < campIds.length; i += 50) {
      ...
      for (const camp of (data?.campaigns || [])) amazonCampById.set(String(camp.campaignId), camp);
    ...
    for (let i = 0; i < targetIds.length; i += 50) {
      ...
      for (const target of (data?.targetingClauses || data?.targets || [])) amazonTargetById.set(String(target.targetId), target);
    ...
    for (const d of keywordBidDecisions) {
      ...
      if (!amz) {
        ...
        await base44.asServiceRole.entities.OptimizationDecision.update(d.id, {
          ...
          error_message: 'Confirmação Amazon falhou: keyword não encontrada.', updated_at: now,
        }).catch(() => {});
        continue;
      }
      ...
      if (diff < 0.01) {
        ...
        const kws = await base44.asServiceRole.entities.Keyword.filter({ amazon_account_id: aid, keyword_id: kwId }, undefined, 1).catch(() => []);
        if (kws[0]) await base44.asServiceRole.entities.Keyword.update(kws[0].id, { bid: amzBid, current_bid: amzBid, synced_at: now }).catch(() => {});
      }
    }
    ...
    for (const d of adGroupBidDecisions) {
      ...
      if (amz) {
        const rows = await base44.asServiceRole.entities.AdGroup.filter({ amazon_account_id: aid, ad_group_id: entityId }, undefined, 1).catch(() => []);
    ...
    for (const d of targetBidDecisions) {
      ...
      if (amz) {
        const rows = await base44.asServiceRole.entities.ProductTarget.filter({ amazon_account_id: aid, target_id: entityId }, undefined, 1).catch(() => []);
    ...
    for (const d of campDecisions) {
      ...
      if (!amz) {
        ...
        await base44.asServiceRole.entities.OptimizationDecision.update(d.id, {
          ...
          error_message: 'Confirmação Amazon falhou: campanha não encontrada.', updated_at: now,
        }).catch(() => {});
        continue;
      }
      ...
      if (['set_budget', 'update_budget', 'reduce_budget', 'increase_budget'].includes(d.action)) {
        ...
        if (amzBudget > 0 && budgetDiff > 0.10) {
          ...
          const camps = await base44.asServiceRole.entities.Campaign.filter({ amazon_account_id: aid, campaign_id: campId }, undefined, 1).catch(() => []);
          if (camps[0]) await base44.asServiceRole.entities.Campaign.update(camps[0].id, { daily_budget: amzBudget, budget: amzBudget, updated_at: now }).catch(() => {});
