...
function classify(campaign: any, product: any, economics: any, terms: any[]) {
  const stock = n(product?.stock_quantity ?? product?.available_quantity);
  const buyable = product?.listing_buyable !== false && product?.buy_box_eligible !== false;
  const costKnown = n(economics?.unit_cost ?? economics?.cost) > 0;
  const margin = n(economics?.margin_percent ?? economics?.margin_pct);
...
Deno.serve(async (request) => {
  ...
  const [campaigns, products, economicRows, terms] = await Promise.all([
    base44.asServiceRole.entities.Campaign.filter(filter, '-updated_at', 10000).catch(() => []),
    base44.asServiceRole.entities.Product.filter(filter, '-updated_at', 10000).catch(() => []),
    base44.asServiceRole.entities.ProductEconomics.filter(filter, '-updated_at', 10000).catch(() => []),
    base44.asServiceRole.entities.SearchTerm.filter(filter, '-last_seen_at', 20000).catch(() => []),
  ]);
  let classified = 0;
  const summary: Record<string, number> = {};
  for (const campaign of campaigns) {
    const asin = text(campaign.asin);
    const product = products.find((row: any) => text(row.asin) === asin || (campaign.sku && text(row.sku) === text(campaign.sku)));
    const economics = economicRows.find((row: any) => text(row.asin) === asin || (campaign.sku && text(row.sku) === text(campaign.sku)));
    const rows = terms.filter((row: any) => String(row.campaign_id || '') === String(campaign.amazon_campaign_id || campaign.campaign_id || ''));
    const state = classify(campaign, product, economics, rows);
    await base44.asServiceRole.entities.Campaign.update(campaign.id, {
      product_lifecycle_stage: state.lifecycle, campaign_journey_stage: state.journey,
      economic_state: state.economic, risk_state: state.risk, opportunity_state: state.opportunity,
      journey_classified_at: new Date().toISOString(),
    });
    classified++;
