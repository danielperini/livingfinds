...
function isWeekendDate(value: string) {
  const d = new Date(`${value}T12:00:00-03:00`); const dow = d.getUTCDay(); return dow === 0 || dow === 6;
}
...
Deno.serve(async (request) => {
  try {
    ...
    const account = accounts[0]; if (!account) return Response.json({ ok: false, error: 'Nenhuma conta conectada' }, { status: 404 });
    const aid = account.id; const clock = dateParts();
    ...
    const holidayDates = new Set<string>(); for (const rule of rules) for (const date of Array.isArray(rule.holiday_dates) ? rule.holiday_dates : []) holidayDates.add(String(date));
    ...
    const perf = settings[0] || {}; const targetAcos = Number(perf.target_acos || 15);
    ...
    if (!evidence || !economicallyHealthy || !lostAuctionOpportunity) {
      return Response.json({ ok: true, skipped: true, reason: !evidence ? 'insufficient_weekend_evidence' : !economicallyHealthy ? 'weekend_economics_not_healthy' : 'no_evidence_of_lost_auction_opportunity', weekend, weekday, revenue_drop_pct: r2(revenueDrop * 100), impression_drop_pct: r2(impressionDrop * 100), cvr_retention_pct: r2(cvrRetention * 100) });
    }
    ...
    if (!existing.length && body.dry_run !== true) {
      await base44.asServiceRole.entities.DaypartingDecision.create({
        amazon_account_id: aid, entity_type: 'account', entity_id: aid, day_of_week: clock.dayOfWeek, hour: clock.hour,
        slot_label: `${clock.dayOfWeek}_${clock.hour}h_weekend_holiday`, time_slot_score: uplift === 20 ? 95 : 80,
        slot_classification: 'STRONG_TIME', rule_id: 'weekend_holiday_learning_signal', rule_version: 'weekend-holiday-learning-v1',
        decision_type: 'MAINTAIN', bid_multiplier: 1 + uplift / 100, metric_window: `${LOOKBACK_DAYS}d`, decision_window: 'current_hour_brt',
        requires_approval: false, status: 'executed', data_confidence: 'HIGH', data_mature: true,
        ...
        idempotency_key: idem, cycle_date: clock.date, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), executed_at: new Date().toISOString(),
      }).catch(() => null);
    }

    const response = body.dry_run === true ? { data: { ok: true, dry_run: true } } : await base44.asServiceRole.functions.invoke('runCanonicalDaypartingEngine', {
      amazon_account_id: aid, bid_multiplier_override: 1 + uplift / 100, trigger_type: 'weekend_holiday_learning', _service_role: true,
    }).catch((error: any) => ({ data: { ok: false, error: error?.message || String(error) } }));
    const engine = response?.data || response || {};
    await base44.asServiceRole.entities.SyncExecutionLog.create({
      amazon_account_id: aid, operation: 'weekend_holiday_daypart_learning', trigger_type: body.trigger_type || 'scheduler', status: engine.ok === false ? 'warning' : 'success',
      records_processed: 1, result_summary: `Sinal sazonal +${uplift}% entregue ao motor único; receita/dia=${r2(revenueDrop * 100)}% abaixo; impressões/dia=${r2(impressionDrop * 100)}% abaixo; CVR_retido=${r2(cvrRetention * 100)}%; ACoS=${r2(Number(weekend.acos))}%.`, started_at: new Date().toISOString(), completed_at: new Date().toISOString(),
    }).catch(() => {});
    return Response.json({ ok: engine.ok !== false, weekend_holiday: true, learned_uplift_cap_pct: uplift, signal_only: true, canonical_engine_executed: body.dry_run !== true, weekend, weekday, engine });
