...
Deno.serve(async (request) => {
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      const targetMer = resolveTargetMer(settingsRows[0], body);
      ...
      for (const metric of campaignMetrics) {
        ...
        const sales = n(metric.sales);
        const orders = n(metric.orders ?? metric.purchases);
        if (spend <= 0) continue;
        const targetAcos = Math.max(1, n(settingsRows[0]?.target_acos ?? settingsRows[0]?.acos_target, 15));
        ...
        const nextBid = roundBid(currentBid * (1 - reduction));
        if (nextBid >= currentBid) continue;
        const key = `blended-mer:${accountId}:${entityType}:${entityId}:${today}:${nextBid.toFixed(2)}`;
        if (activeKeys.has(key)) continue;
        candidates.push({ campaign, campaignId, metric, entity, entityType, entityId, currentBid, nextBid, reduction, spend, sales, orders, acos, key, asin });
      }
      ...
      for (const item of candidates.slice(0, Math.min(MAX_ACTIONS, n(body.max_actions, MAX_ACTIONS)))) {
        const rationale = `Guardrail blended: gasto Ads R$ ${totalSpend.toFixed(2)} / receita total Amazon R$ ${totalRevenue.toFixed(2)} = MER/TACoS ${(mer! * 100).toFixed(2)}%, acima da meta ${(targetMer * 100).toFixed(2)}%. Redução defensiva de ${(item.reduction * 100).toFixed(0)}% aplicada somente em entidade não protegida.`;
      ...
      results.push({ account_id: accountId, date: today, target_mer: targetMer, mer, total_spend: totalSpend, total_revenue: totalRevenue, excess_spend: excessSpend, candidates: candidates.length, actions_created: created, dry_run: body.dry_run === true });
    }

    return Response.json({
      ok: true,
      engine: 'blended-mer-guardrail-v1',
