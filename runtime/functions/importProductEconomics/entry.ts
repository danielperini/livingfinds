...
import { mapAnalyticsSpreadsheetRow } from "../../shared/analyticsSpreadsheetImport.ts";
...
function invalidNonNegativeFields(item: any) {
  ...
  const unitCostError = importedUnitCostError(item);
  if (unitCostError) reasons.push(unitCostError);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (file_url && refresh_amazon_status === true) {
      ...
      // A falha de uma leitura Amazon não pode descartar custos confirmados pelo
      // usuário. Sem status fresco, productActive permanece falso e o repricing
      // fica bloqueado até uma sincronização posterior confirmar a oferta.
    }
    ...
    const existingBySku = new Map(
      [...existingGroups.entries()]
        ...
        .map(([key, records]) => [key, records[0]]),
    );
    const ambiguousExistingSkus = new Set(
      [...existingGroups.entries()]
        .filter(([, records]) => records.length > 1)
        .map(([key]) => key),
    );
    ...
    const ambiguousProductSkus = new Set(
      [...productGroups.entries()]
        .filter(([, records]) => records.length > 1)
        .map(([key]) => key),
    );
    ...
    if (file_url) {
      const extraction: any = await base44.asServiceRole.integrations.Core
              ...
              rows: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    "Custo Unitário Médio": { type: "number" },
                    "Custo Extra (opcional)": { type: "number" },
                    "Preço": { type: "number" },
                    "Unidades Vendidas Totais": { type: "number" },
                    "Vendas Amazon": { type: "number" },
                    "Faturamento": { type: "number" },
                    "Lucro": { type: "number" },
                    "Margem": { type: "number" },
                    "Custo Ads": { type: "number" },
                    "Lucro Pós Ads": { type: "number" },
                    "MPA (Margem Pós ADS)": { type: "number" },
                  },
                },
              },
            },
      ...
      spreadsheetItems = extraction.output.rows.map((row: any) => ({
        ...mapAnalyticsSpreadsheetRow(row, original_file_name),
        repricing_enabled: enable_repricing_for_active === true,
        reason: "Custo confirmado por planilha analítica enviada pelo usuário.",
      }));
    }
    const toProcess = recalculate_only
      ...
      ? items
      : [];
    const importSkuCounts = new Map<string, number>();
    for (const item of toProcess) {
      const key = normalizeSku(item.sku);
      if (key) importSkuCounts.set(key, (importSkuCounts.get(key) || 0) + 1);
    }
    const reportedAmbiguousSkus = new Set<string>();
    ...
    for (const item of toProcess) {
      ...
      const ambiguity = ambiguousExistingSkus.has(normalizedSku)
        ? "mais de um registro ProductEconomics"
        : ambiguousProductSkus.has(normalizedSku)
        ? "mais de um produto no catálogo"
        : (importSkuCounts.get(normalizedSku) || 0) > 1
        ? "mais de uma linha na importação"
        : null;
      if (ambiguity) {
        if (!reportedAmbiguousSkus.has(normalizedSku)) {
          errors.push({
            sku: item.sku,
            error:
              `SKU ambíguo após normalização (${normalizedSku}): ${ambiguity}. Nenhum custo foi aplicado.`,
          });
          reportedAmbiguousSkus.add(normalizedSku);
        }
        continue;
      }
      ...
      const record: any = {
        ...
        sku: product?.sku || previous?.sku || item.sku,
        ...
        cost_source: item._recalculate
          ...
          : item.cost_source || "manual_confirmed",
        ...
        imported_at: now,
        analytics_import_source_file: item.analytics_import_source_file ??
          previous?.analytics_import_source_file ?? null,
        analytics_import_snapshot_date: item.analytics_import_snapshot_date ??
          previous?.analytics_import_snapshot_date ?? null,
        analytics_import_metrics: item.analytics_import_metrics ??
          previous?.analytics_import_metrics ?? null,
      ...
      if (changed && !item._recalculate) {
        await base44.asServiceRole.entities.ProductEconomicsHistory.create({
          ...
          source: item.cost_source || "manual_confirmed",
          reason: item.reason ||
    ...
    return Response.json({
      ...
      amazon_status_warning: file_url && refresh_amazon_status === true &&
          amazonStatusSync?.ok !== true
        ? amazonStatusSync?.error ||
          "Status Amazon indisponível; custos foram salvos, mas o repricing permaneceu bloqueado."
        : null,
    });
  } catch (error: any) {
    return Response.json({
