 ...
 * redirect_uri resolvido dinamicamente via APP_BASE_URL env ou header origin.
 */
import { createClientFromRequest } from 'npm:@base44/sdk@0.8.40';

function resolveRedirectUri(req: Request): string {
  const appBaseUrl = Deno.env.get('APP_BASE_URL')?.trim();
  if (appBaseUrl) return `${appBaseUrl}/amazon-ads-callback`;
  const origin = req.headers.get('origin') || req.headers.get('referer') || '';
  if (origin) {
    try {
      const url = new URL(origin);
      return `${url.origin}/amazon-ads-callback`;
    } catch {}
  }
  return 'https://living-finds-flow.base44.app/amazon-ads-callback';
}
...
Deno.serve(async (req) => {
  try {
    ...
    console.log('[exchangeAmazonAdsCode] redirect_uri usado:', redirectUri);
    ...
    try {
      let accounts: any[] = [];
      // Tentar via usuário autenticado primeiro
      try {
    ...
    // Fire-and-forget: retomar pipeline bloqueada
    if (accountUpdated && savedAccountId) {
      base44.asServiceRole.functions.invoke('checkAndForceReportPipeline', {
        amazon_account_id: savedAccountId,
        force: true,
        _service_role: true,
      }).catch(() => {});
    }
