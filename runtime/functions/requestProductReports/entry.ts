...
Deno.serve(async (req) => {
  try {
    ...
    const rows = await base44.asServiceRole.entities.SyncExecutionLog.filter(
      { amazon_account_id: body.amazon_account_id, operation: 'product_reports_request_v2', status: 'completed' },
      '-completed_at',
      1
    ).catch(() => []);
    ...
    if (last?.completed_at && Date.now() - new Date(last.completed_at).getTime() < 3300000) {
      ...
      if (saved.requested?.length) {
        return Response.json({ ok: true, reused: true, requested: saved.requested, errors: saved.errors || [], requested_at: last.completed_at });
      }
    }

    const result = await base44.asServiceRole.functions.invoke('requestProductReportsV2', {
      ...body,
      _service_role: true,
    });
