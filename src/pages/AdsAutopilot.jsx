...
const TABS = [
  ...
  { id: 'negatives',       label: '🚫 Negativas' },
  { id: 'alerts',          label: '🔔 Alertas' },
  { id: 'history',         label: '📋 Histórico' },
  { id: 'recommendations', label: '🎯 Recomendações' },
  { id: 'ml_learning',     label: '🧠 Motor ML' },
  { id: 'budget',          label: '💰 Budget 30d' },
    ...
    <>
      <tr className={`border-b border-surface-2/40 transition-colors ${selected ? 'bg-cyan/5' : 'hover:bg-surface-2/60'}`}>
        ...
        <td className="px-3 py-3 min-w-[160px] max-w-[200px]">
          {(() => {
            const asin = dec.asin;
            const prod = asin && productMap ? productMap.get(asin) : null;
            if (!prod && !asin) return <span className="text-xs text-slate-600">—</span>;
            return (
              <div className="flex items-center gap-2">
                <div className="flex-shrink-0">
                  {prod?.product_image_url
                    ? <img src={prod.product_image_url} alt="" className="w-7 h-7 rounded object-cover bg-surface-3" />
                    : <div className="w-7 h-7 rounded bg-surface-3 flex items-center justify-center"><Package className="w-3 h-3 text-slate-600" /></div>
                  }
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-mono text-cyan">{asin}</p>
                  <p className="text-[10px] text-slate-400 truncate max-w-[130px] leading-tight">{prod?.product_name || prod?.display_name || ''}</p>
                </div>
              </div>
            );
          })()}
            ...
            <div className="flex items-center gap-2">
              ...
              {editBid ? (
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => setEditBid(false)} className="text-slate-500 hover:text-slate-300"><XCircle className="w-3 h-3" /></button>
                </div>
              ) : (
        ...
        <td className="px-3 py-3 pr-5 w-36">
          <div className="flex items-center gap-1.5">
            <button onClick={() => onApprove(editBid && bidValue ? Number(bidValue) : undefined)}
              ...
              {isLoading ? <><Loader2 className="w-3 h-3 animate-spin" />Agendando...</> : <><CheckCircle className="w-3 h-3" />Aprovar</>}
            </button>
      ...
      {showRationale && (
        <tr className="border-b border-surface-2/40 bg-surface-2/30">
          <td colSpan={7} className="px-10 py-3">
            <p className="text-xs text-slate-400 leading-relaxed italic">💡 {dec.rationale || 'Sem análise disponível.'}</p>
          </td>
        </tr>
      )}
    </>
...
export default function AdsAutopilot() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [cams, allDecs, als, negs, hist, rs, cfgs, prods] = await Promise.all([
        ...
        base44.entities.AutopilotRun.filter({ amazon_account_id: aid }, '-started_at', 5),
      ...
      const dates = [acc.last_sync_at, ...rs.map(r => r.started_at)].filter(Boolean)
  ...
  if (loading) return <div className="p-6 text-slate-400 flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Carregando Autopilot...</div>;
  if (!account) return (
    <div className="p-6">
      <h2 className="text-lg font-bold text-white">Nenhuma conta Amazon conectada.</h2>
      <p className="text-sm text-slate-400 mt-2">Configure sua conta Amazon nas Configurações antes de usar o Autopilot.</p>
    </div>
  );
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
            ...
            {lastUpdate
              ? `Atualizado em ${new Date(lastUpdate).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}`
              : 'Sem registro de execução'
            }
            <span className="text-slate-600">· Automação rodando automaticamente no backend</span>
          </p>
        </div>
      ...
      {error && <div className="text-xs text-red-400 px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg">{error}</div>}

      <AutopilotKPIBar campaigns={campaigns} decisions={decisions} alerts={alerts} runs={runs} config={config} />

      {/* Tabs */}
      <div className="flex flex-wrap gap-1.5 border-b border-surface-2 pb-0">
        {TABS.map(({ id, label }) => (
          <button key={id} onClick={() => setTab(id)}
            className={`px-3 py-2 rounded-t-lg text-xs font-semibold border-b-2 transition-colors whitespace-nowrap ${tab === id ? 'border-cyan text-cyan' : 'border-transparent text-slate-500 hover:text-slate-300'}`}>
            {label}
            {id === 'decisions' && decisions.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[9px] bg-amber-500/20 text-amber-400 rounded-full">{decisions.length}</span>
            )}
            {id === 'alerts' && alerts.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[9px] bg-red-500/20 text-red-400 rounded-full">{alerts.length}</span>
            )}
            {id === 'negatives' && negatives.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[9px] bg-violet-500/20 text-violet-400 rounded-full">{negatives.length}</span>
            )}
          </button>
        ))}
      </div>
            ...
            {decisions.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                <select value={filterType} onChange={e => setFilterType(e.target.value)}
                  className="text-xs bg-surface-2 border border-surface-3 text-slate-300 rounded-lg px-3 py-1.5">
                  <option value="all">Todos os tipos</option>
                  {decisionTypes.map(t => <option key={t} value={t}>{DECISION_LABELS[t] || t}</option>)}
                </select>
                {selectedIds.size > 0 && (
                  <>
                    ...
                    <button onClick={() => bulkAction('approve')} disabled={bulkLoading}
                      className="px-3 py-1.5 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs rounded-lg hover:bg-emerald-500/25 disabled:opacity-50">
                      ✓ Aprovar selecionadas
                    </button>
                    <button onClick={() => bulkAction('reject')} disabled={bulkLoading}
                      className="px-3 py-1.5 bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-lg hover:bg-red-500/20 disabled:opacity-50">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-surface-2 bg-surface-1">
                    <th className="pl-4 py-2 w-8"></th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Decisão</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider min-w-[160px]">Produto</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider w-24">Prioridade</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider w-52">Valor</th>
                    <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider w-24">Confiança</th>
                ...
                <tbody>
                  {filteredDecisions.map(dec => (
                    <DecisionRow key={dec.id} dec={dec} actionState={actionStates[dec.id]}
                      onApprove={(v) => handleDecision(dec.id, 'approve', v)}
                      onReject={() => handleDecision(dec.id, 'reject')}
                      selected={selectedIds.has(dec.id)} onSelect={() => toggleSelect(dec.id)}
                      currencySymbol={currencySymbol} productMap={productMap} />
                  ))}
                </tbody>
              </table>
              {!filteredDecisions.length && (
                <div className="p-6 text-sm text-slate-500 text-center">
                  {decisions.length === 0 ? '✅ Nenhuma decisão pendente — motor rodando automaticamente.' : 'Nenhuma decisão com este filtro.'}
                </div>
              )}
            </div>
          </div>
      ...
      {tab === 'history' && (
        <div className="rounded-xl border border-surface-2 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-surface-2 bg-surface-1">
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Tipo</th>
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Entidade</th>
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Valores</th>
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Var.</th>
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Status</th>
                <th className="px-4 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase">Data</th>
              </tr>
            </thead>
            <tbody>
              {decHistory.map(d => <HistoryRow key={d.id} d={d} currencySymbol={currencySymbol} />)}
            </tbody>
          </table>
          {!decHistory.length && <div className="p-6 text-sm text-slate-500 text-center">Nenhum histórico disponível.</div>}
        </div>
      )}
      ...
      {tab === 'negatives' && (
        <div className="bg-surface-1 border border-surface-2 rounded-xl overflow-hidden">
          <div className="px-5 py-3 border-b border-surface-2 flex items-center justify-between">
            <p className="text-sm font-semibold text-white">Sugestões de Keywords Negativas</p>
            <span className="text-xs text-slate-500">{negatives.length} pendente(s)</span>
          </div>
          {negatives.length === 0 ? (
            <div className="p-8 text-center text-sm text-slate-500">Nenhuma sugestão negativa pendente.</div>
          ) : (
            <div className="divide-y divide-surface-2/50 max-h-[500px] overflow-y-auto">
              {negatives.map(n => (
                <div key={n.id} className="px-5 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-white truncate">{n.keyword || n.search_term || '—'}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">
                      Spend: R${(n.spend || 0).toFixed(2)} · Pedidos: {n.orders || 0}
                    </p>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => approveNegative(n.id)}
                      className="px-2.5 py-1.5 bg-red-500/15 border border-red-500/30 text-red-400 hover:bg-red-500/25 text-xs rounded-lg">
                      Negativar
                    </button>
                    <button onClick={() => rejectNegative(n.id)}
                      className="px-2.5 py-1.5 bg-surface-2 border border-surface-3 text-slate-400 hover:text-slate-200 text-xs rounded-lg">
                      Ignorar
                    </button>
                  </div>
                </div>
