            ...
            <>
              ...
              {filteredDecisions.length === 0 ? (
                ...
                <div className="bg-surface-1 border border-surface-2 rounded-xl overflow-hidden">
                  {/* Barra de ações em massa — sempre visível */}
                  <div className="px-4 py-2.5 border-b border-surface-2 bg-surface-2/40 flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        <span className="text-xs text-slate-400">
                          {selectedIds.size > 0 ? `${selectedIds.size} selecionadas` : `Selecionar todas (${filteredDecisions.length})`}
                        </span>
                      </label>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                        <XCircle className="w-3.5 h-3.5" />
                        Rejeitar selecionadas {selectedIds.size > 0 && `(${selectedIds.size})`}
                      </button>
                      <button
                        onClick={() => bulkAction('approve')}
                        disabled={bulkLoading || selectedIds.size === 0}
                        className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-lg transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                        {bulkLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle className="w-3.5 h-3.5" />}
                        Aprovar selecionadas {selectedIds.size > 0 && `(${selectedIds.size})`}
                      </button>
                      <div className="w-px h-4 bg-surface-3" />
                      <button
                        onClick={() => bulkAll('reject')}
                        disabled={bulkLoading}
                        className="text-xs px-3 py-1.5 bg-surface-2 border border-surface-3 text-slate-400 hover:text-red-400 hover:border-red-500/30 rounded-lg transition-colors disabled:opacity-40">
                        Rejeitar todas
                      </button>
                      <button
                        onClick={() => bulkAll('approve')}
                        disabled={bulkLoading}
                        className="text-xs px-3 py-1.5 bg-surface-2 border border-surface-3 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/30 rounded-lg transition-colors disabled:opacity-40">
                        Aprovar todas
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="px-4 py-2.5 border-t border-surface-2 bg-surface-2/20">
                    <p className="text-xs text-slate-500">{filteredDecisions.length} decisões pendentes · clique no bid proposto para editar antes de aprovar</p>
                  </div>
                </div>
              )}
