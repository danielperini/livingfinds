  ...
  return (
    <>
      <tr className={`border-b border-surface-2/40 transition-colors ${selected ? 'bg-cyan/5' : 'hover:bg-surface-2/60'}`}>
        ...
        <td className="px-3 py-3 pr-5 w-36"><div className="flex items-center gap-1.5"><button onClick={() => onApprove(editBid && bidValue ? Number(bidValue) : undefined)} disabled={isLoading} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-bold rounded-lg disabled:opacity-50 transition-colors whitespace-nowrap">{isLoading ? <><Loader2 className="w-3 h-3 animate-spin" />Agendando...</> : <><CheckCircle className="w-3 h-3" />Aprovar</>}</button><button onClick={onReject} disabled={isLoading} className="flex items-center gap-1 px-2.5 py-1.5 bg-surface-2 hover:bg-red-500/20 border border-surface-3 hover:border-red-500/30 text-slate-400 hover:text-red-400 text-xs font-bold rounded-lg disabled:opacity-50 transition-colors"><XCircle className="w-3 h-3" /></button></div></td>
      </tr>
