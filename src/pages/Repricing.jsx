...
// A análise concorrencial só é útil para ofertas que podem ser vendidas agora.
// O histórico econômico continua preservado para auditoria, mas não é exibido
// como recomendação operacional quando o SKU está inativo ou sem estoque FBA.
const isActiveRepricingProduct = product => product?.status === 'active'
  && Number(product.available_quantity ?? product.fba_inventory ?? 0) > 0;
...
function CompetitorModal({ row, onClose }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <div className="max-h-[88vh] w-full max-w-5xl overflow-hidden rounded-xl border border-violet-500/30 bg-surface-1 shadow-2xl">
      <div className="flex items-start justify-between gap-4 border-b border-surface-2 p-4"><div><h3 className="text-sm font-semibold text-violet-300">Concorrentes identificados — {row.sku}</h3><p className="mt-1 text-xs text-slate-400">{row.title}</p><p className="mt-1 text-[10px] text-slate-600">Até 10 genéricos equivalentes pesquisados pela Zyte em páginas públicas da Amazon · dados agregados e inferidos</p></div><button type="button" onClick={onClose} className="rounded-lg border border-surface-3 p-2 text-slate-400 hover:text-white"><X className="h-4 w-4" /></button></div>
...
export default function Repricing() {
  ...
  const recommendationRows = useMemo(() => {
    ...
    return [...historyCandidates, ...economicsCandidates]
      ...
      .filter(item => isActiveRepricingProduct(item.product));
  ...
  return (
    <div className="min-h-full p-4 md:p-6 space-y-5">
      ...
      {connection && <section className={`rounded-xl border p-4 ${connection.degraded ? 'border-amber-500/20 bg-amber-500/5' : connection.connected ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5'}`}><div className="flex items-start gap-3">{connection.connected ? <CheckCircle2 className={`mt-0.5 h-5 w-5 ${connection.degraded ? 'text-amber-400' : 'text-emerald-400'}`} /> : <Tag className="mt-0.5 h-5 w-5 text-red-400" />}<div className="min-w-0"><h2 className={`text-sm font-bold ${connection.degraded ? 'text-amber-400' : connection.connected ? 'text-emerald-400' : 'text-red-400'}`}>{connection.degraded ? 'SP-API conectada com quota temporariamente limitada' : connection.connected ? 'Repricing conectado à Amazon SP-API' : 'Conexão SP-API incompleta'}</h2><p className="mt-1 text-xs text-slate-400">{connection.message || (connection.connected ? 'OAuth, Listings Items e Product Pricing foram validados sem alterar preços.' : 'Confira os detalhes abaixo.')}</p>{connection.checks && <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-5">{Object.entries(connection.checks).map(([name, check]) => <div key={name} className="rounded-lg border border-surface-3 bg-surface-1/60 p-2"><p className="text-[10px] font-semibold uppercase text-slate-500">{name}</p><p className={`mt-1 text-[10px] ${check.ok ? 'text-emerald-400' : check.degraded || check.skipped ? 'text-amber-400' : 'text-red-400'}`}>{check.message}</p></div>)}</div>}</div></div></section>}
      ...
      <section className="rounded-xl border border-violet-500/20 bg-surface-1">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-surface-2 p-4"><div><h2 className="text-sm font-semibold text-violet-300">Análise de preços da concorrência</h2><p className="mt-1 text-[10px] text-slate-500">Somente SKUs ativos, com estoque FBA disponível. Até 10 genéricos equivalentes por SKU, pesquisados pela Zyte no back-end e classificados por produto, modelo, cor e tamanho.</p><p className="mt-1 text-[10px] text-emerald-400">Consultas idênticas usam cache por 6 horas. O preço recomendado continua protegido internamente por margem líquida ≥15% e confiança superior a 95%.</p></div><span className="rounded-full border border-violet-500/20 bg-violet-500/10 px-3 py-1 text-xs font-semibold text-violet-300">{recommendationRows.length} SKUs ativos</span></div>
      ...
      <section className="rounded-xl border border-surface-2 bg-surface-1">
        ...
        <div className="max-h-[520px] overflow-auto"><table className="w-full text-xs"><thead className="sticky top-0 z-10 bg-surface-2"><tr className="text-left text-[10px] uppercase tracking-wider text-slate-500">{skuColumns.map(([key, label]) => <th key={key} className="whitespace-nowrap px-4 py-3"><button type="button" onClick={() => changeSkuSort(key)} className="inline-flex items-center gap-1 hover:text-cyan">{label}{skuSort.key === key ? skuSort.direction === 'asc' ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" /> : <span className="text-slate-700">↕</span>}</button></th>)}</tr></thead><tbody>{allSkuRows.map(product => {
          ...
          const statusLabel = activeAndBuyable ? 'Ativo e comprável' : hasActiveInventory && (!listingChecked || product.listing_status_confirmed !== true) ? 'Estoque ativo · oferta em validação' : hasActiveInventory ? 'Estoque ativo · oferta bloqueada' : product.inventory_status === 'out_of_stock' ? 'Sem estoque' : product.listing_status === 'not_found' ? 'Listing não encontrado' : 'Não comprável confirmado';
