...
const ACTION_LABELS = {
  ...
  negative_keyword: '🚫 Negativar Keyword',
  pause_ad_group: '⏸️ Pausar Ad Group',
  no_action: '✅ Sem Ação',
};
...
function DecisionRow({ d, onUpdate }) {
  ...
  const isCritical = ['pause_campaign', 'enable_campaign', 'negative_keyword'].includes(d.action);
  const changePct = d.change_pct || 0;
  const isIncrease = changePct > 0;

  const approve = async () => {
    setLoading(true);
    await base44.entities.AutopilotDecision.update(d.id, { status: 'approved' });
  ...
  const reject = async () => {
    setLoading(true);
    await base44.entities.AutopilotDecision.update(d.id, { status: 'rejected' });
    onUpdate(d.id, 'rejected');
    setLoading(false);
  };

  const execute = async () => {
    if (isCritical && !window.confirm(`Confirma executar "${ACTION_LABELS[d.action]}" em "${d.entity_name}"?`)) return;
    setLoading(true);
  ...
  if (d.status !== 'pending' && d.status !== 'approved') return null;

  return (
    <>
      <tr className="border-b border-surface-2/40 hover:bg-surface-2/50 transition-colors">
        <td className="px-4 py-3">
          <div className="text-xs font-semibold text-white truncate max-w-[180px]">{d.entity_name || d.entity_id}</div>
          <div className="text-xs text-slate-500 mt-0.5">{ACTION_LABELS[d.action] || d.action}</div>
        </td>
        <td className="px-3 py-3">
          <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${RISK_COLORS[d.risk_level]}`}>
            {d.risk_level === 'low' ? 'Baixo' : d.risk_level === 'medium' ? 'Médio' : 'Alto'}
          </span>
        </td>
        <td className="px-3 py-3">
          {d.current_value != null && (
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-mono text-slate-400">${Number(d.current_value).toFixed(2)}</span>
              ...
              <span className="text-xs font-mono text-white">${Number(d.new_value || 0).toFixed(2)}</span>
            </div>
          )}
        </td>
        <td className="px-3 py-3 max-w-[200px]">
          <button onClick={() => setExpanded(v => !v)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300">
            ...
            <span className="truncate max-w-[150px]">{d.reason}</span>
          </button>
        </td>
      ...
      {expanded && (
        <tr className="border-b border-surface-2/40 bg-surface-2/20">
          <td colSpan={6} className="px-8 py-2.5">
            ...
            {d.evidence && <p className="text-xs text-slate-500 mt-0.5">📊 {d.evidence}</p>}
          </td>
        </tr>
      )}
    </>
...
export default function AutopilotDecisionsTable({ decisions, onRefresh }) {
  ...
  const approveAll = async () => {
    setBulkLoading(true);
    const pendingLow = localDecisions.filter(d => d.status === 'pending' && d.risk_level === 'low');
  ...
  const counts = {
    ...
    rejected: localDecisions.filter(d => d.status === 'rejected' || d.status === 'failed').length,
  };
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-surface-2 flex-wrap gap-2">
        <div className="flex gap-1">
          {[
            ...
            { k: 'rejected', label: `Rejeitadas (${counts.rejected})` },
          ].map(t => (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-surface-2 bg-surface-2/50">
              {['Entidade', 'Risco', 'Valor Atual → Novo', 'Motivo', 'Status', 'Ação'].map(h => (
