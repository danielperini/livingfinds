  ...
  return (
    <div className={`bg-surface-1 border rounded-xl p-4 ${tones[tone]}`}>
      <div className="flex items-center gap-1.5 mb-1">
        {KpiIcon ? <KpiIcon className="w-3 h-3 text-slate-500" /> : null}
      ...
      <button onClick={onToggle} className="w-full flex items-start gap-3 px-5 py-3 hover:bg-surface-2/30 transition-colors text-left">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {dec.asin ? <span className="text-[10px] font-mono text-cyan flex-shrink-0">{dec.asin}</span> : null}
      ...
      {expanded ? (
        <div className="px-5 pb-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2 text-[10px]">
            {dec.settings_source ? (
              <span className="px-2 py-1 bg-surface-2 rounded-lg text-slate-500">Fonte: {dec.settings_source}</span>
            ) : null}
            {dec.data_quality ? (
              <span className={`px-2 py-1 rounded-lg ${dec.data_quality === 'fresh' ? 'bg-emerald-500/10 text-emerald-400' : dec.data_quality === 'acceptable' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400'}`}>
                Dados: {dec.data_quality}
              </span>
            ) : null}
            {dec.stock_coverage_days != null ? (
              <span className="px-2 py-1 bg-surface-2 rounded-lg text-slate-500">Estoque: {Math.round(dec.stock_coverage_days)}d</span>
            ) : null}
            {dec.created_at ? (
...
export default function EstrategiasTab({ account }) {
  ...
  const strategicMetrics = useMemo(() => {
    ...
    const recentDecisions = decisions.filter(d => d.created_at && (Date.now() - new Date(d.created_at).getTime()) < 24 * 3600000);
    const intentDist = {};
    for (const d of decisions) {
      if (d.search_intent_type) intentDist[d.search_intent_type] = (intentDist[d.search_intent_type] || 0) + 1;
    }
    return {
      totalSpend, totalSales, totalOrders, globalAcos, globalRoas,
      highAcosCount, noConversionCount,
  ...
  const filteredDecisions = useMemo(() => {
    return decisions.filter(d => {
      ...
      if (searchTerm) {
        ...
        if (!(d.keyword_text || '').toLowerCase().includes(q) && !(d.asin || '').includes(q) && !(d.rationale || '').toLowerCase().includes(q)) return false;
      }
      return true;
    });
        ...
        <div className={`rounded-xl border p-4 space-y-3 ${result.ok ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
          ...
          {result.economic_context ? (
            <div className="flex flex-wrap gap-2 text-[10px]">
              ...
              <span className="px-2 py-1 bg-surface-2 rounded-lg text-slate-400">
                {String(result.economic_context.products_with_dynamic_target)} produtos com meta dinâmica
              </span>
          ...
          {result.stats ? (
            <div className="flex flex-wrap gap-3 text-[10px] text-slate-500">
              <span>Avaliadas: {result.stats.evaluated ?? 0}</span>
              <span>Protegidas: {result.stats.protected ?? 0}</span>
              <span>Aumentos: {result.stats.bid_increase ?? 0}</span>
              <span>Reduções: {result.stats.bid_reduce ?? 0}</span>
              <span>Sem dados: {result.stats.held ?? 0}</span>
            </div>
          ) : null}
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          ...
          {!lastEngineResult ? (
            <div className="bg-cyan/5 border border-cyan/20 rounded-xl p-3 flex items-center gap-2 text-xs text-cyan/80">
              ...
              Execute o motor de estratégias para ver os visibility scores calculados.
            </div>
          ) : null}
                          ...
                          return (
                            <tr key={i} className="border-b border-surface-2/40 hover:bg-surface-2/20 transition-colors">
                              <td className="px-3 py-2.5">
                                <p className="text-slate-200 font-medium truncate max-w-[140px]">{opp.keyword_text || '—'}</p>
                                {opp.asin ? <p className="font-mono text-cyan text-[9px]">{opp.asin}</p> : null}
                              </td>
                              <td className="px-3 py-2.5">
                                <span className={`inline-flex px-1.5 py-0.5 rounded text-[9px] font-semibold ${stateCfg.bg} ${stateCfg.color}`}>{stateCfg.label}</span>
                              </td>
                              <td className="px-3 py-2.5">
                              ...
                              <td className="px-3 py-2.5"><span className={`font-semibold ${confColor}`}>{opp.growth_confidence || '—'}</span></td>
                              <td className="px-3 py-2.5">
      ...
      {(activeView === 'strategic' && !loading) ? (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {strategicMetrics.highAcosCount > 0 ? (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/8 border border-red-500/20 text-xs">
                  ...
                  <span className="text-red-300"><strong>{strategicMetrics.highAcosCount}</strong> campanha(s) com ACoS acima da meta</span>
                </div>
              ) : null}
              {strategicMetrics.noConversionCount > 0 ? (
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-amber-500/8 border border-amber-500/20 text-xs">
                  ...
                  <span className="text-amber-300"><strong>{strategicMetrics.noConversionCount}</strong> campanha(s) gastando sem converter</span>
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          {Object.keys(strategicMetrics.intentDist).length > 0 ? (
            <div className="bg-surface-1 border border-surface-2 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
                <Brain className="w-4 h-4 text-violet-400" /> Distribuição de Intenção de Busca — Decisões
              </h3>
              <div className="space-y-2">
                {Object.entries(strategicMetrics.intentDist).sort(([, a], [, b]) => b - a).map(([intent, count]) => {
          ...
          {products.length > 0 ? (
            <div className="bg-surface-1 border border-surface-2 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
                <Layers className="w-4 h-4 text-cyan" /> Produtos por Estado Estratégico
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {Object.entries(
      ...
      {activeView === 'decisions' ? (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="flex gap-1 bg-surface-2 border border-surface-3 rounded-lg p-0.5">
              {[{ k: 'all', l: 'Todos' }, { k: 'approved', l: 'Aprovadas' }, { k: 'executed', l: 'Executadas' }, { k: 'failed', l: 'Falharam' }].map(f => (
          ...
          {loading ? (
            ...
            <div className="bg-surface-1 border border-surface-2 rounded-xl overflow-hidden">
              {filteredDecisions.slice(0, 100).map(dec => (
                <DecisionRow key={dec.id} dec={dec} expanded={expandedIds.has(dec.id)} onToggle={() => toggleExpand(dec.id)} />
              ))}
                        ...
                        {products.slice(0, 50).map(p => {
                          ...
                          return (
                            <tr key={p.id} className="border-b border-surface-2/40 hover:bg-surface-2/20">
                              <td className="px-3 py-2"><p className="font-mono text-cyan">{p.asin}</p><p className="text-slate-500">{p.sku}</p></td>
          ...
          {perfSettings ? (
            ...
            <div className="bg-surface-1 border border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">O motor está usando valores padrão.</p>
              </div>
              <Link to="/settings" className="ml-auto px-3 py-2 bg-amber-500/15 border border-amber-500/30 text-amber-300 text-xs font-semibold rounded-lg whitespace-nowrap">
                Configurar
              </Link>
            </div>
          )}
