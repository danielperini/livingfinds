...
export default function ProfitProjectionPanel({ account }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [daily, productRows] = await Promise.all([
        base44.entities.DailyProductAdsAssessment.filter({ amazon_account_id: account.id }, '-assessment_date', 5000).catch(() => []),
        base44.entities.Product.filter({ amazon_account_id: account.id }, '-updated_at', 5000).catch(() => []),
      ]);
  ...
  const model = useMemo(() => {
    ...
    const currentAsins = new Set(products.filter((product) =>
      product.asin && product.status === 'active' &&
    ...
    const currentRows = rows.filter((row) => scope.has(row.assessment_date) && currentAsins.has(row.asin));
    const recentDates = new Set(latestDates.slice(-30));
    const sellingAsins = new Set(currentRows.filter((row) =>
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h2 className="flex items-center gap-2 text-base font-bold text-white"><Calculator className="h-4 w-4 text-cyan" /> Projeção de Lucro</h2><p className="mt-0.5 text-xs text-slate-400">Base empírica: {model.dates.length} dias de aferição, somente SKUs ativos/com estoque e com venda nos últimos 30 dias. Novos dias entram automaticamente no cálculo.</p></div>
    ...
    <div className="grid gap-3 rounded-xl border border-cyan/20 bg-cyan/5 p-4 md:grid-cols-2">
      <label className="text-xs text-slate-300">Meta de lucro mensal (R$)<input type="number" min="0" value={monthlyGoal} onChange={(event) => setMonthlyGoal(Number(event.target.value))} className="mt-1.5 w-full rounded-lg border border-surface-3 bg-surface-1 px-3 py-2 text-sm text-white outline-none focus:border-cyan" /></label>
