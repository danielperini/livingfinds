  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto scrollbar-thin">
      ...
      {issues > 0 ? (
        ...
        <div className="rounded-xl border border-cyan/20 bg-cyan/5 px-4 py-3 text-xs text-slate-300">
          <strong className="text-cyan">Decisão V4:</strong>{' '}
          {stats.highAcos > 0 ? `${stats.highAcos} campanha(s) acima de 40% entram em redução/pausa econômica; ` : ''}
          {stats.noOrders > 0 ? `${stats.noOrders} com gasto sem venda entram em proteção; ` : ''}
          {stats.noSpend > 0 ? `${stats.noSpend} sem gasto entram em recuperação de entrega/rebuild em lotes de até 20.` : ''}
          {' '}Campanha nova só é criada com termo relevante e sem duplicidade; vencedoras não são pausadas.
        </div>
        </div>
      ) : (
