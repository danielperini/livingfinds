    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-2xl border border-surface-2 bg-surface-1">
        ...
        <div className="p-5 space-y-5">
          ...
          <section><h3 className="text-xs font-semibold text-slate-300 uppercase mb-2">Ofertas equivalentes do mesmo ASIN</h3><div className="grid grid-cols-1 md:grid-cols-3 gap-2">{offers.length ? offers.map((offer, index) => <div key={`${offer.sellerId}-${index}`} className="rounded-lg border border-surface-3 p-3 text-xs"><p className="font-bold text-slate-200">{money(offer.totalPrice)}</p><p className="text-slate-500">{offer.condition || 'New'} · {offer.fulfillmentType || 'n/d'} {offer.isFeatured ? '· Featured Offer' : ''}</p></div>) : <p className="text-xs text-slate-500">Nenhuma oferta equivalente recente disponível.</p>}</div><p className="mt-2 text-[10px] text-slate-600">Fonte: {economics?.competition_source || 'pendente'} · consulta {dateTime(economics?.competition_checked_at)}</p></section>
