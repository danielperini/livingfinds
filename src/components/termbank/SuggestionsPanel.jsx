    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>{suggestions.map((s) => {
          ...
          const done = ['approved', 'created'].includes(s.status) || s.queue_status === 'scheduled' || s.queue_status === 'completed';
          return <tr key={s.id} className="border-b border-surface-2/40">
            ...
            <td className="px-4 py-3 text-xs text-amber-400">{done ? 'programado 00h–04h' : (s.status || 'suggested')}</td>
            <td className="px-4 py-3"><div className="flex gap-2">
              <button disabled={busy || done} onClick={() => onReview(s, 'approve')} className="flex items-center gap-1 rounded-lg bg-emerald-500/15 px-3 py-1.5 text-xs font-semibold text-emerald-300 disabled:opacity-70">{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Check className="h-3 w-3" />}{done ? 'Realizado' : 'Aprovar'}</button>
              {!done && <button disabled={busy} onClick={() => onReview(s, 'delete')} className="flex items-center gap-1 rounded-lg bg-red-500/10 px-3 py-1.5 text-xs font-semibold text-red-300 disabled:opacity-40"><Trash2 className="h-3 w-3" />Excluir</button>}
            </div></td>
          </tr>;
        })}</tbody>
      </table>
