...
import { Loader2, ExternalLink, CheckCircle, XCircle, Clock, AlertTriangle, Zap, Target } from 'lucide-react';
...
export default function AmazonSuggestionsTab({ suggestions, products, account, onRefresh }) {
  ...
  const handleFetchSuggestions = async () => {
    ...
    try {
      ...
      const d = res?.data;
      if (d?.ok) {
        setMessage({ type: 'success', text: `✓ ${d.total_created} sugestões importadas da Amazon Ads. ${d.total_skipped} já existiam.` });
        onRefresh();
      } else {
