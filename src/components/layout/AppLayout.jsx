...
import {
  ...
  Tag, Bot, Activity, Bell, Search, ShieldCheck, CircleDollarSign
} from 'lucide-react';
...
export default function AppLayout() {
  ...
  return (
    <div className="lf-app-shell flex h-screen overflow-hidden text-[var(--text-primary)]">
      ...
      <aside
        className={`lf-sidebar fixed lg:relative z-50 h-full flex flex-col border-r transition-all duration-200 ease-out
      ...
      >
        <div className={`flex items-center h-[72px] px-4 ${collapsed ? 'justify-center' : 'justify-between'}`}>
          ...
          {!collapsed ? (
            <button
              ...
              className="lf-icon-button hidden lg:flex items-center justify-center w-8 text-slate-500 hover:text-slate-200"
        ...
        <nav className="flex-1 px-2.5 pb-4 overflow-y-auto scrollbar-thin" aria-label="Navegação principal">
          {navGroups.map((group) => (
            <div key={group.label} className="mb-5">
              {!collapsed ? (
                <p className="px-3 mb-2 text-[10px] font-semibold tracking-[0.12em] uppercase text-slate-600">
                  {group.label}
                </p>
              ) : null}
                ...
                {group.items.map(({ path, icon: Icon, label, sub }) => {
                  ...
                  return (
                    <Link
                      ...
                      to={path}
                      onClick={() => setMobileOpen(false)}
                      className={`lf-nav-link flex items-center gap-3 px-3 text-slate-400 hover:text-slate-100
                        ${active ? 'lf-nav-link-active' : ''}
        ...
        <div className="p-3 space-y-2 border-t border-white/[0.06]">
          <Link
            to="/settings"
            className={`lf-nav-link flex items-center gap-3 px-3 text-slate-400 hover:text-slate-100 ${location.pathname.startsWith('/settings') ? 'lf-nav-link-active' : ''} ${collapsed ? 'justify-center' : ''}`}
          ...
          {!collapsed ? (
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.025] p-3">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-xs font-medium text-slate-200 truncate">Conta Amazon</p>
                  <p className="text-[10px] text-slate-500 mt-0.5 truncate">{account?.name || account?.profile_name || 'Conta conectada'}</p>
                </div>
            ...
            <button
              ...
              className="lf-icon-button w-full flex items-center justify-center text-slate-400 hover:text-white"
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="lf-topbar min-h-[72px] flex items-center gap-3 px-4 md:px-6 border-b flex-shrink-0">
          <button
            type="button"
            className="lf-icon-button lg:hidden w-10 flex items-center justify-center text-slate-400 hover:text-slate-100"
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-lg md:text-xl font-semibold text-white tracking-[-0.025em] truncate">{currentMeta[0]}</h1>
            <p className="hidden sm:block text-xs text-slate-500 mt-0.5 truncate">{currentMeta[1]}</p>
          </div>

          <div className="hidden xl:flex lf-search h-10 w-[320px] items-center gap-2 px-3" role="search">
            <Search className="w-4 h-4 text-slate-500" />
            <span className="text-xs text-slate-500">Busca disponível nas páginas com dados</span>
          </div>

          <button
            type="button"
            className="lf-icon-button relative w-10 flex items-center justify-center border border-white/[0.06] bg-white/[0.025] text-slate-400 hover:text-white"
          ...
          <Link
            to="/sala-de-comando"
            className="lf-primary-button flex items-center gap-2 px-3.5 text-xs font-semibold whitespace-nowrap"
          >
